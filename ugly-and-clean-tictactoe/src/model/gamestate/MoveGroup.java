...
public class MoveGroup {
	...
	public void add(Move move) {
		if (move.getScore() == MoveScore.NONE.getScore()) return;
		if (move.getPosition() == -1) return;
	...
	public Move bestOverallMove() {
		if (allMoves.size() == 0) return new Move(-1, MoveScore.NONE, "Couldnt find a decent move");
	...
	public Move getTwoBirdsMove(Move highestScoringMove, ArrayList<Move> allMoves) {
		ArrayList<Move> bestScoringMoves = getAllMovesWithSameScoreAsHighestScoringMove(allMoves, highestScoringMove);
	...
	public Move findMoveThatRepeatsMost(ArrayList<Move> bestMoves) {
		...
		for (int i = 1; i < bestMoves.size(); i++) {
			...
			bestMove = saveThisMoveAsNewBestMoveIfPossible(moveA, numberOfBestMoveOccurrences, numberOfThisMovesOccurrences);
		}
		
		return bestMove;
	}

	private Move saveThisMoveAsNewBestMoveIfPossible(Move moveA, int numberOfBestMoveOccurences, int numberOfThisMovesOccurences) {
	...
	private int calculateNumberOfMoveRecurrences(Move moveA) {
		...
		for (int j = 0; j < allMoves.size(); j++) {
			numberOfMoveRecurrences = incrementMoveOccurrencesIfMoveRecurs(moveA, numberOfMoveRecurrences, j);
		}
		
		return numberOfMoveRecurrences;
	}

	private int incrementMoveOccurrencesIfMoveRecurs(Move moveA, int numberOfThisMovesOccurences, int j) {
	...
	public ArrayList<Move> getAllMovesWithSameScoreAsHighestScoringMove(ArrayList<Move> allMoves, Move highestScoringMove) {
		ArrayList<Move> bestMoves = new ArrayList<Move>();
		
		for (int i = 0; i < allMoves.size(); i++) {
			bestMoves = addValidMoveIfSameScoreAsHighestScoringMove(highestScoringMove, bestMoves, i);
		}
		
		return bestMoves;
	}

	private ArrayList<Move> addValidMoveIfSameScoreAsHighestScoringMove(Move highestScoringMove, ArrayList<Move> bestMoves, int i) {
	...
	public void add(ArrayList<Integer> positions, MoveScore score, String message) {
