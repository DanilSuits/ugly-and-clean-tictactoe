...
public class MakeMoveTests extends MockObjectTestCase {
	...
	public void testFindTesseraEndingBlockingPositionBeforePairStartingPosition() throws Exception {
		...
		board.setPosition(3,2, Board.THEIR_PLAYER_MARK);
		board.setPosition(3,3, Board.OUR_PLAYER_MARK);
		board.setPosition(3,4, Board.OUR_PLAYER_MARK);
		board.setPosition(3,5, Board.OUR_PLAYER_MARK);
		board.setPosition(3,6, Board.OUR_PLAYER_MARK);
	...
	public void testFillGapInPotentialOpposingTessera() throws Exception {
		board.setPosition(4,7, Board.THEIR_PLAYER_MARK);
		board.setPosition(5,6, Board.THEIR_PLAYER_MARK);
		board.setPosition(7,3, Board.THEIR_PLAYER_MARK);
		board.setPosition(7,4, Board.THEIR_PLAYER_MARK);
	...
	public void testViewUpdatedWithNewMarkOnMove() throws Exception {
		...
		checking(new Expectations() {{
			...
			one(mockGameView).drawMark(1, 2, Board.THEIR_PLAYER_MARK);
			
			// Allow any choice for the selection of the computer's position
			one(mockGameView).drawMark(with(any(int.class)), with(any(int.class)), with(equal(Board.OUR_PLAYER_MARK)));
		}});
		game = new TicTacToeGame(new PatrickStrategy(), mockGameView);
		game.makeCompleteMoveCycle(12);
	}
	
	 // TODO: test that the game updates the view with the spot where the last move was
}
