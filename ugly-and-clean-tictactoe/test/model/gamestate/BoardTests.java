...
public class BoardTests extends TestCase {
	...
	public void testCanAndGetSetPositionsWithSingleIntegerRepresentation() throws Exception {
	...
	public void testConvertOneDimensionalBoardtoTwoDimensionalBoard() throws Exception {
	...
	public void testTooBigABoardBlowsUp() throws Exception {
		...
		try {
			...
			assertEquals("Incoming one-dimensional board array must be of length " + Board.MAX_BOARD_SIZE, e.getMessage());
		}
	}
	
	public void testCanGetOppositePlayerMarkForEitherPlayerMark() throws Exception {
		...
		assertEquals(playerMark, board.getOppositePlayerMarkFor(otherPlayerMark));
	}
	...
	public void testPositionIsOutsideBoardBoundaries() throws Exception {
		assertFalse(board.within1DBoardPositionIndexBounds(Board.MAX_BOARD_SIZE + 10));
	}
	
	public void testRowIsOutsideBoardBoundaries() throws Exception {
		assertFalse(board.within2DBoardPositionIndexBounds(Board.MAX_ROWS + 5, Board.MAX_COLUMNS - 1));
		assertFalse(board.within2DBoardPositionIndexBounds(-5,  Board.MAX_COLUMNS - 1));
	}
	
	public void testColumnIsOutsideBoardBoundaries() throws Exception {
		assertFalse(board.within2DBoardPositionIndexBounds(Board.MAX_ROWS -1, Board.MAX_COLUMNS + 5));
		assertFalse(board.within2DBoardPositionIndexBounds(Board.MAX_ROWS -1,  - 5));
	}
	
	public void testPositionIsWithinBoardBoundaries() throws Exception {
		assertTrue(board.within1DBoardPositionIndexBounds(Board.MAX_BOARD_SIZE - 10));
		assertTrue(board.within2DBoardPositionIndexBounds(Board.MAX_ROWS - 1, Board.MAX_COLUMNS - 1));
		assertTrue(board.within2DBoardPositionIndexBounds(Board.MAX_ROWS - 1, Board.MAX_COLUMNS - 1));
	}
	
	public void test1DEmptyPositionCheckOutsideBoardBoundaries() throws Exception {
		try {
			board.positionIsEmpty(Board.MAX_BOARD_SIZE + 10);
		} catch (Exception e) {
			assertEquals("Board index 110 is outside board bounds.", e.getMessage());
		}
	}
	
	public void test2DEmptyPositionCheckOutsideBoardBoundaries() throws Exception {
		try {
			board.positionIsEmpty(Board.MAX_ROWS + 5, Board.MAX_COLUMNS - 1);
		} catch (Exception e) {
			assertEquals("Board position 15,9 is outside board bounds.", e.getMessage());
		}
	}
	...
	public void test2DSetPositionOutsideBoardBoundaries() throws Exception {
		try {
			board.setPosition(-5, Board.MAX_COLUMNS - 1, Board.THEIR_PLAYER_MARK);
		} catch (Exception e) {
			assertEquals("Board index -5,9 is outside board bounds.", e.getMessage());
		}
	}
	...
	public void testSetIncomingBoardArray() throws Exception {
		...
		TicTacToeGame game = new TicTacToeGame(new PatrickStrategy(), new StubView());
