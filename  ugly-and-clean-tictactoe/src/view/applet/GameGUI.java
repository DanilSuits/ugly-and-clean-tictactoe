...
public class GameGUI extends Applet implements GameView {
	...
	// We maintain an offscreen image buffer, so as to not have to maintain or query total game state on every repaint
	...
	public void handleMouseEvent(int mouseX, int mouseY) {
		if (gameNotInPlay(mouseX, mouseY)) return;
		
		int playerPosition = getPlayerPosition(mouseX, mouseY);
		if (!game.positionIsAvailable(playerPosition)) return;
	...
	private boolean gameNotInPlay(int mouseX, int mouseY) {
		...
		if (cannotMakeAMove(mouseX, mouseY)) return true;
		return false;
	}

	private boolean cannotMakeAMove(int mouseX, int mouseY) {
		if (game.weHaveAWinner()) return true;
		if (game.moveNumber() > TicTacToeGame.MAX_NUMBER_OF_MOVES) return true;
	...
	private void resetToolbar() {
		...
		imageGraphics.drawImage(gameImages.getYourTurnImage(), Board.MAX_BOARD_SIZE, 0, this);
	}

	public void gameIsADraw() {
		throw new RuntimeException("DAVE: Implement me, do we have an image for a draw?");
	}

	public void theyWonGame() {
		imageGraphics.drawImage(gameImages.getWinningImage(), Board.MAX_BOARD_SIZE, 0, this);
	}

	public void weWonGame() {
		imageGraphics.drawImage(gameImages.getLosingImage(), Board.MAX_BOARD_SIZE, 0, this);
	}
	...
	public void drawMark(final int row, final int column, int playerMark) {
		...
		imageGraphics.drawImage(gameImages.getImageForPlayerMark(playerMark), x, y, this);
	...
	private void scheduleUpdateLastPlayedImage(final int x, final int y) {
		// Update the last played image to change from a filled o mark to a normal o mark after 1/2 a second.
		
		TimerTask timerTask = new TimerTask() {
			@Override
			public void run() {
				imageGraphics.drawImage(gameImages.getOMark(), x, y, GameGUI.this);
				repaint();
			}
		};
