...
public class Board {
	public static class AccessException extends RuntimeException {
		...
		public AccessException(int row, int column) {
			...
			boolean columnOffender = column < MIN_COLUMN_INDEX || column > MAX_COLUMN_INDEX;
			boolean rowOffender = row < MIN_COLUMN_INDEX;
			if (columnOffender) {
				offender = Offender.COLUMN;
				if (rowOffender) offender = Offender.BOTH;
			}
		}
	...
	private int[][] convertOneDimensionaltoTwoDimensionalBoardArray(int[] oneDimensionalBoardArray) {
		if (oneDimensionalBoardArray.length != MAX_BOARD_SIZE) {
			throw new RuntimeException("Incoming one-dimensional board array must be of length " + MAX_BOARD_SIZE);
		}
	...
	private int[][] convertAllRowsInBoard(int[] oneDimensionalBoardArray) {
		...
		for (int row = 0; row < MAX_ROWS; row++) {
			incomingBoardArrayIndex = convertAllSquaresInRow(oneDimensionalBoardArray, incomingBoardArrayIndex, row);
		}
		
		return tempBoardArrayForDimensionConversions;
	}

	private int convertAllSquaresInRow(int[] oneDimensionalBoardArray, int incomingBoardArrayIndex, int row) {
	...
	public void setPosition(int row, int column, int playerMark) {
		if (!within2DBoardPositionIndexBounds(row, column)) throw new RuntimeException("Board index " + row + "," + column + " is outside board bounds.");
		boardArray[row][column] = playerMark;
	}

	public int getPosition(int singleIndex) {
		if (!within1DBoardPositionIndexBounds(singleIndex)) throw new RuntimeException("Board index " + singleIndex + " is outside board bounds.");
	...
	private boolean allRowsInBoardAreEmpty() {
		for (int row = 0; row < MAX_ROWS; row++) {
			if (!allSquaresInRowAreEmpty(row)) return false;
		}
		return true;
	}
	...
	private boolean allSquaresInRowAreEmpty(int row) {
		for (int column = 0; column < MAX_COLUMNS; column++) {
			if (boardArray[row][column] != EMPTY) return false;
		}
		return true;
	}
	...
	public boolean positionIsEmpty(int trialPosition) {
		if (!within1DBoardPositionIndexBounds(trialPosition)) throw new RuntimeException("Board index " + trialPosition + " is outside board bounds.");
	...
	public boolean positionIsEmpty(int row, int column)  {
		if (!within2DBoardPositionIndexBounds(row, column)) throw new RuntimeException("Board position " + row + "," + column + " is outside board bounds.");
		return boardArray[row][column] == EMPTY;
	}
	...
	public ArrayList<Integer> getAllPositionsOccupiedByPlayer(int theirPlayerMark) {
		...
		for (int row = 0; row < MAX_ROWS; row++) {
			getAllOccupiedPositionsInRow(theirPlayerMark, allPlayerPositions, row);
		}
	...
	private void getAllOccupiedPositionsInRow(int theirPlayerMark, ArrayList<Integer> allPlayerPositions, int row) {
		for (int column = 0; column < MAX_COLUMNS; column++) {
			addOccupiedPosition(theirPlayerMark, allPlayerPositions, row, column);
		}
	}
	...
	public boolean within2DBoardPositionIndexBounds(int row, int column) {
		if ((row < 0 )    || (row > Board.MAX_ROWS - 1)) return false;
		if ((column < 0 ) || (column > Board.MAX_COLUMNS - 1)) return false;
