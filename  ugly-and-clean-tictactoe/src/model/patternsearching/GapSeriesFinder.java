...
public class GapSeriesFinder extends SeriesFinder implements ISeriesFinder {
	...
	protected ISeries searchIndexListForSeriesOfSize(IndexList currentIndexList, SeriesSize expectedSize, int playerMark) {
		...
		for (int i = 0; i < currentIndexList.size() - expectedSize.getSize() + 1; i++) {
			seriesFound = addSeriesIfItQualifies(getNextCandidateListOfExpectedSize(currentIndexList, i));
		}
	...
	private ISeries addSeriesIfItQualifies(IndexList candidateIndexList) {
		...
		int lastPosition = candidateIndexList.get(candidateIndexList.size() - 1);
	...
	private ISeries qualifyingGapSeries(SeriesSize expectedSize, IndexList candidateIndexList) {
		if ((candidateListContainsBoardMark(candidateIndexList, Board.EMPTY) 
				&& (!candidateListContainsBoardMark(candidateIndexList, board.getOppositePlayerMarkFor(playerMark))))) {
	...
	private ISeries createGapSeriesIfIndexListIsOfExpectedSize(IndexList candidateIndexList) {
	...
	protected IndexList getNextCandidateListOfExpectedSize(IndexList currentIndexList, int currentStartingPositionIndex) {
		IndexList candidateList = new IndexList();
		for (int j = currentStartingPositionIndex; j < currentStartingPositionIndex + expectedSize.getSize(); j++) {
	...
	private boolean weHaveASeries(int firstPosition, int lastPosition) {
		return (positionEquals(firstPosition, playerMark)) && positionEquals(lastPosition, playerMark);
	}
	...
	protected boolean candidateListContainsBoardMark(IndexList candidateList, int mark) {
	...
	private boolean positionContainsBoardMark(int positionIndex, int mark, IndexList candidateList) {
